...
就是一个数组以 `[`（LB 左中括号）开始，`]`（RB 右中括号）结束。值之间使用 `,`（COMMA,逗号）分隔。
...
public List<Object> parseArray() throws JSONException {
    ...
    if(token.tokenType!= RB){
        throw new NoViableTokenException("expected token is "+ token.tokenType + "but actual is " + forward.tokenType);
    }
    return  objectList;
}
```
...
其实这里的代码或者是原本的逻辑的问题多多的。因为原来的 `value` 函数中是这样的。进行判断的
```java
private Object value() throws JSONException {
    Token token = input.getNextToken();
    switch (token.tokenType) {
...
而 `parseArray`函数为了判断空数组的情况。就会先用 input.getNextToken 拿字符串的后一个 Token
...
如何不是空数组比如是 `[1,2]` ,`parseArray` 先来一个 nextToken，此时 Token 会是`2-NUM`，如果不是。运行`value`函数的时候又 nextToken，value 函数中的数字就变成`,-COMMA`，就不能正确解析。
...
private List<Object> parseArray() throws JSONException {
    ...
    if(forward.tokenType!= RB){
        throw new NoViableTokenException("expected token is "+ forward.tokenType + "but actual is " + forward.tokenType);
    }
    return  objectList;
}
```
...
下面再重构下代码。
有没有发现，有好多 `forward = input.getNextToken()` 和 `if(forward.tokenType!=RB) throws Expection`这样的代码。
这样的代码看起来也不太好看。
...
private void match(TokenType tokenType) throws JSONException {
    if(forward.tokenType == tokenType){
        ...
        throw new NoViableTokenException("expected token is " +  tokenType + "but actual is " + forward.tokenType);
    }
}
```
